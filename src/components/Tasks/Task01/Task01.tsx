...
function Task01(props: IProps) {
    ...
    const move = (clientY: number, clientX: number, device: string) => {
        if (targetDrag) {
            ...
            if (device === "phone") {
                y = clientY - stateY.current - (targetDrag.offsetHeight / 2);
                x = clientX - stateX.current - (targetDrag.offsetWidth / 2);
            } else {
                y = clientY - stateY.current - (targetDrag.offsetHeight / 2) + 5;
                x = clientX - stateX.current - (targetDrag.offsetWidth / 2);
            }
